...
export default class ElementScores extends Closable(LitElement) {
  ...
  async willUpdate(changedProperties: PropertyValues) {
    if (changedProperties.has("gameState")) {
      if (
        this.gameState.country &&
        this.leaderboard?.collection !== this.gameState.country
      ) {
      ...
      if (this.leaderboard && this.userInfo) {
        this.allowedToSubmitScore = await this.leaderboard.allowedToSubmitScore(
          this.userInfo,
          this.gameState,
        );
      }
  ...
  render() {
    return html`
      <wa-dialog label="${this.localize.term("game_over")}">
        <div class="wa-stack wa-gap-2xl">
          <wa-callout variant="brand">
            ...
            <strong>${gameScore(this.gameState)}</strong>
          </wa-callout>
          <!-- FIXME: check allowedToSubmitScore -->
          <wa-button
            class="save_score"
            variant="brand"
            @click="${this.saveScore}"
            ?disabled="${!!this.userInfo.username &&
            !this.allowedToSubmitScore}"
          >
      ...
      <element-username
        ...
        @username="${this.handleUsernameSet}"
      ></element-username>
    `;
  }
  ...
  private async saveScoreToLeaderboard() {
    if (this.allowedToSubmitScore) {
      await this.leaderboard.saveScore(
        this.userInfo.userId,
        this.userInfo.username,
        gameScore(this.gameState),
      );
      this.allowedToSubmitScore = await this.leaderboard.allowedToSubmitScore(
        this.userInfo,
        this.gameState,
